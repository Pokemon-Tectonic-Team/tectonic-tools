...
if (process.env.NODE_ENV === "development") {
    console.debug("[TectonicData] Running in development mode: including pockets 9-13 as held items");
    TectonicData.heldItems = Object.values(TectonicData.items).filter((x) => (x.pocket >= 9 && x.pocket <= 13));
} else {
    console.debug("[TectonicData] Running in production mode: only pocket 5 as held items");
